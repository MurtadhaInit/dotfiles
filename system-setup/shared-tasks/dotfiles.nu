...
def symlink_dotfiles [] {
  ...
  if not ($env.PWD | str ends-with ".dotfiles/Applications") {
    print "⚠️ Couldn't navigate to the correct directory to symlink"
    exit 1
  }

  let dirs = ls | where type == dir | get name
  if ($dirs | is-empty) {
    print "⚠️ No applications in .dotfiles/Applications to symlink"
    return
  }
  ...
  try {
    ...
    print "⚠️ Couldn't symlink dotfiles"
    exit 1
  }
}
